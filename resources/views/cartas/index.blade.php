...
<body>
  ...
  <section class="catalog-header animate__animated animate__fadeInDown">
    <h1 class="display-4 fw-bold">Cartas Digitales</h1>
    <p class="lead">Envía sentimientos con estilo: románticas, de apoyo, agradecimiento y más.</p>
  </section>

  <div class="container py-5">
    <div class="text-center mb-4 animate__animated animate__fadeInUp">
      <button class="btn btn-outline-danger filter-btn active" data-filter="all">Todas</button>
      @foreach($categorias as $clave => $cat)
        <button class="btn btn-outline-danger filter-btn" data-filter="{{ $clave }}">{{ $cat['nombre'] }}</button>
      @endforeach
    </div>
      ...
      @foreach ($categorias as $clave => $cat)
        <div class="col-lg-3 col-md-4 col-sm-6 filtrable category-{{ $clave }} animate__animated animate__zoomIn">
          <div class="card card-hover h-100 text-center">
            <img src="{{ asset('images/cartas/' . $cat['imagen']) }}" 
            ...
            <div class="card-body">
              ...
              <a href="{{ route('cartas.show', $clave) }}" class="btn btn-danger">Ver cartas</a>
            </div>
          </div>
        </div>
      @endforeach
